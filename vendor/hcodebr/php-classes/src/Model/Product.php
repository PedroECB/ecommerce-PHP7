...
public function checkPhoto(){
 ...
 if(file_exists($_SERVER['DOCUMENT_ROOT']. 
  ...
  DIRECTORY_SEPARATOR."products".$this->getidproduct().".jpg" )){
...
public function setPhoto($file){
  ...
  switch($extension){
    ...
    case "gif": $image = imagecreatefromgif($file['tmp_name']);

      break;

    case "png": $image = imagecreatefrompng($file['tmp_name']);
...
public function getFromURL($desurl){
 ...
 $sql = new Sql();
 $rows = $sql->select("SELECT * FROM tb_products WHERE desurl=:desurl limit 1", array(":desurl"=>$desurl));
...
public function getCategories(){
  ...
  return $sql->select("SELECT * FROM tb_categories a INNER JOIN tb_productscategories b ON a.idcategory = b.idcategory
     WHERE b.idproduct=:idproduct", array(":idproduct"=>$this->getidproduct()));
}
